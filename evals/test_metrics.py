...
def case(candidate_summary: str, direction: str = "down") -> EvaluationCase:
    return EvaluationCase.from_dict({
        ...
        "expected": {
            ...
            "qa": [{"question": "Risk nedir?", "answer": "Enerji maliyeti", "source_pages": [2]}],
        },
        "candidate": {
            "summary": candidate_summary,
            "qa": [{"question": "Risk nedir?", "answer": "Enerji maliyeti", "sources": ["Sayfa 2, paragraf 1"]}],
        },
    })
...
def test_single_digit_expected_number_is_measured():
    result = evaluate_case(EvaluationCase.from_dict({
        "id": "single-digit",
        "source_text": "[Sayfa 1]\nGelir 8 milyon TL oldu.",
        "expected": {"facts": [], "numbers": [{"value": "8", "context": "gelir milyon TL"}]},
...
def numeric_case(value: str, context: str, summary: str, direction: str | None = None):
    return EvaluationCase.from_dict({
        "id": "numeric",
        "source_text": "",
        "expected": {"facts": [], "numbers": [{
            "value": value, "context": context, "direction": direction,
        }]},
...
def test_turkish_decimal_comma_matches_decimal_point():
    result = evaluate_case(numeric_case("45,5", "faaliyet gideri yüzde", "- Faaliyet gideri yüzde 45.5 oldu."))
    assert result["numeric_accuracy"] == 1


def test_magnitude_word_is_normalized():
    result = evaluate_case(numeric_case("12 milyon", "uygulama indirme", "- Uygulama indirme sayısı 12.000.000 oldu."))
    assert result["numeric_accuracy"] == 1


def test_turkish_number_word_matches_digit():
    result = evaluate_case(numeric_case("3", "stratejik plan temel eksen", "- Stratejik plan üç temel eksen üzerine kurulmuştur."))
    assert result["numeric_accuracy"] == 1


def test_same_number_in_wrong_context_does_not_match():
    result = evaluate_case(numeric_case(
        "12 milyon", "uygulama indirme",
...
def test_direction_is_checked_in_same_claim_as_number():
    result = evaluate_case(numeric_case(
        "20", "gelir yüzde", "- Gelir yüzde 20 arttı.\n- Maliyet azaldı.", direction="down",
    ))
    assert result["numeric_accuracy"] == 0
...
def test_numeric_details_explain_missing_value():
    result = evaluate_case(numeric_case(
        "1965", "Igor Ansoff Corporate Strategy", "- Alfred Chandler 1962 yılında çalışmasını yayımladı."
    ))
...
def test_compound_overview_is_split_into_independent_claims():
    result = evaluate_case(EvaluationCase.from_dict({
        ...
        "expected": {"facts": []},
        "candidate": {"summary": "Kurum iki anlama sahiptir. Planlama 1980'lerde önem kazandı."},
    }))
...
def test_low_overlap_related_claim_is_uncertain_not_unsupported():
    result = evaluate_case(EvaluationCase.from_dict({
        ...
        "expected": {"facts": []},
        "candidate": {"summary": "- Teknolojik kapasitenin geliştirilmesi planlanmaktadır."},
    }))
    assert result["contradicted_claim_rate"] == 0
...
def test_number_in_second_source_sentence_does_not_create_false_contradiction():
    result = evaluate_case(EvaluationCase.from_dict({
        ...
        "expected": {"facts": []},
        "candidate": {"summary": "- Arşiv 12 milyon indirilirken Bilim Genç 45,5 milyon görüntülendi."},
    }))
    assert result["contradicted_claim_rate"] == 0
