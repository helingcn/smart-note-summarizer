...
from state import clear_summary_state
from summary_utils import error_detail


def render_history(history_data: list[dict], api_url: str, auth_headers: Callable[[], dict]) -> None:
    privacy_left, privacy_right = st.columns([3, 2])
    with privacy_left:
        st.caption("Geçmiş, SmartDigest sunucusundaki kullanıcı hesabınıza özel veritabanında tutulur.")
    with privacy_right:
        clear_confirmed = st.checkbox("Tüm kayıtları silmeyi onaylıyorum", key="clear_history_confirm")
        if st.button("Tüm geçmişi sil", type="secondary", disabled=not clear_confirmed, use_container_width=True):
            try:
                requests.delete(f"{API_URL}/history", headers=auth_headers(), timeout=10).raise_for_status()
    ...
    search_query = st.text_input("Geçmişte ara", placeholder="Özetlerde ara…", label_visibility="collapsed")
    filtered_data = [item for item in history_data if not search_query or search_query.lower() in item["summary"].lower()]
    ...
    for item in filtered_data:
        ...
        with action1:
            st.download_button("Özeti indir", item["summary"], f"ozet_{item_id}.txt", key=f"download_{item_id}")
        with action2:
            confirm_key = f"confirm_delete_{item_id}"
            if st.session_state.get(confirm_key):
                if st.button("Emin misin?", key=f"delete_confirm_{item_id}", type="primary"):
                    try:
                        requests.delete(f"{API_URL}/history/{item_id}", headers=auth_headers(), timeout=10).raise_for_status()
        ...
        with action3:
            if item.get("has_source"):
                ...
                if source_key in st.session_state:
                    st.download_button(
                        "Kaynağı indir", st.session_state[source_key],
                        f"kaynak_{item_id}.txt", key=f"download_source_{item_id}",
                    )
                elif st.button("Kaynağı getir", key=f"fetch_source_{item_id}"):
                    try:
                        source_response = requests.get(
                            f"{API_URL}/history/{item_id}/source", headers=auth_headers(), timeout=10
                        )
