...
DATA_DIR = Path(
    os.getenv("SMARTDIGEST_DATA_DIR", str(PROJECT_DIR / ".smartdigest-data"))
).expanduser().resolve()
DATA_DIR.mkdir(parents=True, exist_ok=True)
DB_PATH = Path(os.getenv("SMARTDIGEST_DB_PATH", str(DATA_DIR / "smartdigest.db"))).expanduser().resolve()
LOG_PATH = Path(os.getenv("SMARTDIGEST_LOG_PATH", str(DATA_DIR / "smartdigest.log"))).expanduser().resolve()
FERNET_KEY_PATH = Path(
    os.getenv("SMARTDIGEST_FERNET_KEY_PATH", str(DATA_DIR / ".smartdigest.key"))
).expanduser().resolve()
JWT_SECRET_PATH = Path(
    os.getenv("SMARTDIGEST_JWT_SECRET_PATH", str(DATA_DIR / ".smartdigest.jwt_secret"))
).expanduser().resolve()
